...
<html lang="fr">
	<head>
		...
		<link rel="stylesheet" type="text/css" href="Chichas.css">
	</head>
	<body>
		<div id="coteg">
			<img src="chicha.svg">
		...
		<div id="prod">
			<?php
				...
				$sql = "SELECT id, name, price, img_url, type FROM Produits WHERE type='Chichas'";
		...
		if (!$_GET["cat"])
		{
				...
				{
					 while ($row = mysqli_fetch_assoc($result))
						 echo "<div class='product'><IMG ALT='image' SRC=".$row["img_url"]." HEIGHT='300' WIDHT='300'>
						 <h1>".$row["name"]."</h1><h2>".$row["price"].
						 "$</h2><form action='ajout_panier.php' method='get'><input name='id' value=".$row["id"]." type='hidden'/><button class='button' type='submit'>Ajouter au panier</button></form></div>";
				}
			}
